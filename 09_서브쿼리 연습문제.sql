...
FROM
    employees e
WHERE e.salary > (SELECT AVG(salary) FROM employees WHERE job_id = 'IT_PROG');
-- 문제 2.
...
FROM
    (SELECT 
        ROWNUM AS rn, des.* 
    FROM 
        (SELECT * FROM employees ORDER BY first_name DESC) des
    )
...
-- 입사일을 출력하세요.
SELECT
    rn, employee_id, first_name, phone_number, hire_date 
FROM
    (SELECT 
        ROWNUM AS rn, a.* 
    FROM 
        (SELECT * FROM employees ORDER BY hire_date ASC) a
    )
